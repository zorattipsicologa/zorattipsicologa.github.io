...
You can create new projects by adding new Markdown files in a `_projects` directory (not included by default in this site). The easiest way to do this is to enable the `projects` collection in [\_config.yml](_config.yml) and copy the pattern used by the `news` collection.
...
You can easily create your own collections, apps, short stories, courses, or whatever your creative work is. To do this, edit the collections in the [\_config.yml](_config.yml) file, create a corresponding folder, and create a landing page for your collection, similar to the existing `news` collection.
